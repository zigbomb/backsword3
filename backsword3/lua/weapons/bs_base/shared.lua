--[[
	Do not share or re-distribute without permission of its author, zigbomb (zig.publicmail@gmail.com)
	BackSword3 was created by zigbomb.

	Documentation on the base can be found below.
--]]
...
SWEP.BulletTracerFreq = 5 -- Show a tracer every x bullets.
...
function SWEP:Reload()
	...
	if (self.ReloadSound) then 
		self.Weapon:EmitSound(self.Primary.Reload) -- Include this in your SWEP if it uses a compiled weapon sound that's one file.
		end
	end
end
...
function SWEP:PrimaryAttack()
	...
	if (self.Owner:IsPlayer()) then
		...
		local flashPos = trace.StartPos + trace.Normal * 32
	end

	local trace = self.Owner:GetEyeTraceNoCursor()

	if (CLIENT) then -- Dynamic lights when the player fires.
		...
		if (dynamicLight) then
			...
			dynamicLight.Pos = trace.StartPos + trace.Normal * 48
			dynamicLight.r = 227;
			dynamicLight.g = 198;
			dynamicLight.b = 102;
		else
	...
	self.Owner:ViewPunch(
		Angle(math.Rand(-0.2, -0.1) * self.Primary.Recoil, math.Rand(-0.1, 0.1) * self.Primary.Recoil, 0) -- Make the player's screen shake based on the amount of recoil.
	);
...
function SWEP:DrawHUD()
	...
	surface.DrawRect(ScrW() / 2.0 -1, ScrH() / 2.0 -1, 2, 2) -- Draws a small dot at the center of the screen.
	return
end
